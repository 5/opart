	...
	public static void main(final String... arguments) {
		...
		if (art.border > 0) {
			try (final ProgressBar bordering = progress("Bordering", 1)) {
				final double diameter = art.size * art.fraction;
				final double margin = (art.size - diameter) / 2d;
				final Area border = new Area(new Ellipse2D.Double(margin - 8, margin - 8, diameter + 16, diameter + 16));
				final Area content = new Area(new Ellipse2D.Double(margin, margin, diameter, diameter));
				border.subtract(content);
				graphics.setColor(Color.BLACK);
				graphics.setPaintMode();
