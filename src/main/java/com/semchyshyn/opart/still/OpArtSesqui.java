...
public final class OpArtSesqui {
	private static final String CLASS_NAME = OpArtSesqui.class.getSimpleName();
	private static final int DEFAULT_SIZE = 18 * 600;
	private static final double DEFAULT_FRACTION = Math.sqrt(2d / Math.PI);
	private static final int DEFAULT_CIRCLES = 16;
	private static final double DEFAULT_TBD = .5d;
	...
	@Option(name = "--circles", metaVar = "CIRCLES", usage = "Number of circles to draw in the circle")
	private int circles = DEFAULT_CIRCLES;
	@Option(name = "--tbd", metaVar = "TBD", usage = "Diameter of the circles as a fraction of the size")
	private double tbd = DEFAULT_TBD;
	...
	private static ProgressBar progress(final String task, final int steps) {
		return new ProgressBar(task, steps, 1, System.err, ProgressBarStyle.ASCII, "", 1L, false, null, null, 0L, Duration.ZERO);
	}

	public static void main(final String... arguments) {
		System.err.println(CLASS_NAME + " (c) 2020-2025 Yuriy Semchyshyn");

		OpArtSesqui art = null;
		...
		try (final ProgressBar parsing = progress("Parsing", 1)) {
			...
			Preconditions.checkArgument(1 <= art.circles, "At least 1 circle must be drawn on the circle");
			// Preconditions.checkArgument(1 <= art.tbd, "The waves must be rotated at least 1 time");
			parsing.step();
		...
		try (final ProgressBar rendering = progress("Rendering", art.circles)) {
			...
			final double angle = 2d * Math.PI / art.circles;

			final double inner = art.size * art.fraction * art.tbd / 2d;

			final double centers = radius - inner;

			for (int circle = 0; circle < art.circles; circle++) {
				Point2D qwe = new Point2D.Double(center - centers * Math.sin(angle * circle),
				                                 center - centers * Math.cos(angle * circle));

				final Shape cir = new Ellipse2D.Double(qwe.getX() - inner, qwe.getY() - inner, qwe.getX() + inner, qwe.getY() + inner);
