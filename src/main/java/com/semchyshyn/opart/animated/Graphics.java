...
public class Graphics {
	...
	public void square(final double apothem) {
		square(Mathematics.ORIGIN,
		       apothem);
	}

	public void square(final @NonNull Point2D center,
	...
	public void square(final double abscissa,
	                   final double ordinate,
	                   final double apothem) {
		if (Mathematics.EPSILON <= apothem) {
			final double side = apothem * 2d;
			final Rectangle2D square = new Rectangle2D.Double(abscissa - apothem,
			                                                  ordinate - apothem,
			                                                  side, side);
	...
	public void triangle(final @NonNull Point2D first,
	                     final @NonNull Point2D second,
	                     final @NonNull Point2D third) {
		final Path2D triangle = new Path2D.Double(Path2D.WIND_EVEN_ODD, 3);
		triangle.moveTo( first.getX(),  first.getY());
		triangle.lineTo(second.getX(), second.getY());
		triangle.lineTo( third.getX(),  third.getY());
		triangle.closePath();
