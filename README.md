...
## Workflow
 ...
 6. ### Imagine
	...
	public class Piece extends OpArt {
		protected final @NonNull Value<Double> radius = new Value<>(Range.UNIT_INTERVAL, Change.DECELERATING, Type.LINEAR_DOUBLE);
	
		@Override
		protected void render(final @NonNull Graphics graphics, final double time) {
			graphics.circle(radius.at(time));
		}
 ...
 8. ### Publish
	Publish the new piece of optical art by manually uploading it to major video sharing platforms, such as
	  * [YouTube](https://www.youtube.com/@an_op_art) (supports preferred `VCodec.FFV1` and `ACodec.FLAC`) or
	  * [Instagram](https://www.instagram.com/an_op_art) (requires supported `VCodec.H264` and `ACodec.AAC`);

	as of late 2025, no other social platforms can be recommended for sharing videos, namely 
	  * ~~[Twitter](https://www.twitter.com)~~ (enforces excessive compression with noticeable visual artifacts) nor
	  * ~~[TikTok](https://www.tiktok.com)~~ (automatically deletes account and ignores support requests)
